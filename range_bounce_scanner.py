"""
Range Bounce Scanner v1.0 — Модуль поиска монет в ценовом коридоре

Ищет монеты в рендже 2–15% с признаками накопления у нижней границы:
  - Повышенный объём у нижней границы (Big Size)
  - Возраст диапазона ≥ 60 минут (Aged 1h+)
  - 3–9 отскоков от нижней границы (Rejected xN)
...
RBS_DEFAULTS = {
    ...
    "min_trades_per_min":  5,             # Мин. сделок/мин
...
class RangeResult:
    ...
    bounces: int                  # кол-во подтверждённых отскоков
    ...
    trades_per_min: float = 0.0    # сделок в минуту
...
class RangeBounceScanner:
    ...
    def analyze_symbol(self, symbol: str, cfg: dict) -> Optional[RangeResult]:
        ...
        # ── Активность ──
        tf_min = TIMEFRAME_MINUTES.get(cfg["timeframe"], 5)
        # Сделок за последние 5 минут (из последних 5m/tf свечей)
        last_candles_5m = max(1, 5 // tf_min)
        recent_df = df.tail(last_candles_5m)
        total_trades_5m = int(recent_df["trades"].sum())
        ...
        bounce_events, bounces = _count_bounces(df, range_low, lower_touch, bounce_confirm, cfg)
...
def render_range_bounce_tab(client, trades_buffer=None):
    ...
    Вызывать из app.py:
        ...
        render_range_bounce_tab(client, st.session_state.trades_buffer)
    """
    st.markdown("### 📊 Range Bounce Scanner v1.0")
    st.caption(
        "Ищет монеты в ценовом коридоре 2–15% с признаками накопления у нижней границы. "
    ...
    with st.expander("⚙️ Параметры сканирования", expanded=False):
        ...
        with c2:
            ...
            min_tpm = st.slider("Мин. сделок/мин", 1, 50, 5, key="rbs_tpm")
        ...
        with c3:
            ...
            min_bounces = st.slider("Мин. отскоков", 1, 5, 3, key="rbs_minb")
    ...
    for r in filtered:
        ...
        rows.append({
            ...
            "Отскоки":   f"{r.bounces}/{r.max_bounces if hasattr(r,'max_bounces') else 9}",
            "Возраст":   age_str,
            "Сделок/м":  f"{r.trades_per_min:.1f}",
    ...
    if sel_result:
        ...
        dc4.metric("Отскоков",       sel_result.bounces)
        ...
        dc7.metric("Сделок/мин",     f"{sel_result.trades_per_min:.1f}")
